...
class PreyCapturesEveryTimesteps(Metric):

    """
    Stores the number of captures every N timesteps.
    """

    def __init__(self, timestep_interval, verbose=False, log_interval=10):

        super(PreyCapturesEveryTimesteps, self).__init__(f"Prey Captures Every {timestep_interval} timesteps")
    ...
    def __call__(self, timestep):
        ...
        if self._timestep_counter % self._timestep_interval == 0:
            ...
            self._current_captures = 0
            if self._verbose: print(self._captures, flush=True)
